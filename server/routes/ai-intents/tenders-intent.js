...
async function handleTenders(user) {
  ...
  const { rows } = await pool.query(
    `SELECT title, client, value, status, submission_date, award_date FROM tenders WHERE organization_id = $1 OR (organization_id IS NULL AND company_id = $2) ORDER BY created_at DESC`,
    [orgId, companyId]
  );
  if (!rows.length) {
    return {
