...
async function handleSafety(user) {
  ...
  const { rows } = await pool.query(
    `SELECT type, title, severity, status, project, date FROM safety_incidents WHERE organization_id = $1 OR (organization_id IS NULL AND company_id = $2) ORDER BY created_at DESC`,
    [orgId, companyId]
  );
  if (!rows.length) {
    return {
