...
router.get('/google', (req, res, next) => {
  const redirectUri = `${process.env.FRONTEND_URL || 'http://localhost:5173'}/dashboard`;
...
router.get('/google/callback', (req, res, next) => {
  passport.authenticate('google', { session: false }, async (err, user, info) => {
    try {
      ...
      const redirectUri = req.query.state || `${process.env.FRONTEND_URL || 'http://localhost:5173'}/dashboard`;
