...
DB_CONTAINER_NET="$(docker inspect -f '{{range $k,$v := .NetworkSettings.Networks}}{{println $k}}{{end}}' cortexbuild-db 2>/dev/null | awk '/cortexbuild/ {print; exit}' || true)"
[ -z "${DB_CONTAINER_NET:-}" ] && DB_CONTAINER_NET="$(docker inspect -f '{{range $k,$v := .NetworkSettings.Networks}}{{println $k}}{{end}}' cortexbuild-db 2>/dev/null | awk 'NF && $0 != "bridge" {print; exit}' || true)"
