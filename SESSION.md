...
### Current position
- **Working tree**: clean at `HEAD` — includes `SESSION.md` handoff, `docs/RALPH_DEEPWORK.md`, `CLAUDE.md` PATH note (checkpoint commit above).
- **Optional manual follow-up**: Align `.github/workflows/cortexbuildpro-ci.yml` build job to `node-version-file: '.nvmrc'` (was `22.x`); optional `cache-dependency-path` + `node -v` sanity step on `ci.yml` if IDE policy allows editing Actions.
