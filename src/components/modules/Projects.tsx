...
import {
  ...
  Loader2, RefreshCw, MessageSquare, Download, FileSpreadsheet, Printer,
} from 'lucide-react';
import {
  ...
  printWindow.document.write(`
    <html>
      ...
      <body>
        ${content}
        <div class="footer">
          Generated by CortexBuild Ultimate on ${new Date().toLocaleString()}
        </div>
        ...
        <div className="flex items-start justify-between mb-4">
          ...
          <div className="flex items-center gap-2 ml-4 flex-shrink-0">
            ...
            <div className="relative group">
              <button className="flex items-center gap-1.5 px-3 py-1.5 bg-orange-600 hover:bg-orange-700 text-white text-sm rounded-lg transition-colors">
                <Download className="w-3.5 h-3.5"/> Export
              </button>
              <div className="absolute right-0 top-full mt-1 bg-gray-800 border border-gray-700 rounded-lg shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50 min-w-[160px]">
                <button onClick={() => exportToCSV([project], `project_${pName.replace(/\s+/g, '_')}`)} className="w-full flex items-center gap-2 px-4 py-2 text-sm text-gray-300 hover:bg-gray-700 hover:text-white rounded-t-lg">
                  <FileSpreadsheet className="w-4 h-4"/> Export CSV
                </button>
                <button onClick={() => exportToJSON([project], `project_${pName.replace(/\s+/g, '_')}`)} className="w-full flex items-center gap-2 px-4 py-2 text-sm text-gray-300 hover:bg-gray-700 hover:text-white">
                ...
                <button onClick={() => {
                  const html = `
                    ...
                    <p><strong>Status:</strong> ${statusCfg.label}</p>
                    <p><strong>Budget:</strong> ${fmtM(budget)}</p>
                    <p><strong>Spent:</strong> ${fmtM(spent)} (${pct.toFixed(1)}%)</p>
                    <p><strong>Progress:</strong> ${progress}%</p>
                    <p><strong>Start Date:</strong> ${project.startDate || project.start_date || 'N/A'}</p>
                    <p><strong>End Date:</strong> ${project.endDate || project.end_date || 'N/A'}</p>
                    <p><strong>Manager:</strong> ${project.manager || 'N/A'}</p>
                    <h2>Financial Summary</h2>
                    <table>
                      <tr><th>Metric</th><th>Value</th></tr>
                      <tr><td>Total Invoiced</td><td>${fmtM(totalInvoiced)}</td></tr>
                      <tr><td>Total Paid</td><td>${fmtM(totalPaid)}</td></tr>
                      <tr><td>Approved CO Value</td><td>${fmtM(approvedCOVal)}</td></tr>
                      <tr><td>Open RFIs</td><td>${openRFIs}</td></tr>
                      <tr><td>Open Safety Issues</td><td>${openSafety}</td></tr>
                    </table>
                  `;
                  printReport(pName, html);
                }} className="w-full flex items-center gap-2 px-4 py-2 text-sm text-gray-300 hover:bg-gray-700 hover:text-white rounded-b-lg">
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          {/* Export buttons */}
          <div className="relative group">
            <button className="flex items-center gap-2 px-3 py-2 bg-gray-800 hover:bg-gray-700 rounded-lg text-white text-sm transition-colors">
              <Download className="w-4 h-4"/>Export
            </button>
            <div className="absolute right-0 top-full mt-1 bg-gray-800 border border-gray-700 rounded-lg shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50 min-w-[160px]">
              <button onClick={() => exportToCSV(projects.map(p => ({...p, budget: Number(p.budget||0), spent: Number(p.spent||0), progress: Number(p.progress||0)})), 'all_projects')} className="w-full flex items-center gap-2 px-4 py-2 text-sm text-gray-300 hover:bg-gray-700 hover:text-white rounded-t-lg">
                <FileSpreadsheet className="w-4 h-4"/>Export CSV
              </button>
              <button onClick={() => exportToJSON(projects, 'all_projects')} className="w-full flex items-center gap-2 px-4 py-2 text-sm text-gray-300 hover:bg-gray-700 hover:text-white">
                <Download className="w-4 h-4"/>Export JSON
              </button>
              <button onClick={() => {
                ...
                const html = `
                  ...
                  <table>
                    <tr><th>Metric</th><th>Value</th></tr>
                    <tr><td>Total Projects</td><td>${projects.length}</td></tr>
                    <tr><td>Active Projects</td><td>${counts.active}</td></tr>
                    <tr><td>Total Budget</td><td>${fmtM(totalBudget)}</td></tr>
                    <tr><td>Total Spent</td><td>${fmtM(totalSpent)}</td></tr>
                    <tr><td>Total Workers</td><td>${totalWorkers}</td></tr>
                  </table>
                  <h2>Project Details</h2>
                  <table>
                    <tr><th>Name</th><th>Client</th><th>Status</th><th>Budget</th><th>Progress</th></tr>
                    ${projects.map(p => `<tr><td>${p.name||''}</td><td>${p.client||''}</td><td>${p.status||''}</td><td>${fmtM(Number(p.budget||0))}</td><td>${p.progress||0}%</td></tr>`).join('')}
                  </table>
                `;
                printReport('Projects Report', html);
              }} className="w-full flex items-center gap-2 px-4 py-2 text-sm text-gray-300 hover:bg-gray-700 hover:text-white rounded-b-lg">
                <Printer className="w-4 h-4"/>Print Report
              </button>
            </div>
