...
## 2026-05-08 - N+1 Queries in RAG Context Retrieval (/api/rag/context)
**Learning:** Running an independent `SELECT` database query for every item matched during RAG context retrieval causes an N+1 query problem, creating significant backend bottleneck due to multiple database roundtrips.
**Action:** Group the target row IDs by table name and execute a single batched query per table using PostgreSQL's `ANY($1)` with an array parameter, significantly reducing latency. Include a fallback to individual queries to handle partial failures.
